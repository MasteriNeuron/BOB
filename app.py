...
import Constants
from Data_Collection import  get_channel_Ids, get_comments, get_playlist_Ids, get_video_details, download_video

app = Flask(__name__)
app.config["SESSION_PERMANENT"] = False
app.config["SESSION_TYPE"] = "filesystem"
Session(app)
app.secret_key = "atanu"
...
def check():
    try:
        ...
        if len(youtube_url) != 0:
            session['channelId'] = get_channel_Ids.get_channel_Ids(youtube_url=youtube_url)
            if session['channelId'] == 'INVALID':
                return redirect(url_for("home"))
            else:
                return render_template("index.html", status_comments="channelId found: "+session['channelId'])
...
def download():
    videoId = request.args.get('videoId')
    file = download_video.download_video(videoId=videoId)
    return file
